...
def launch_fill_from_scalar(scalar, shape, scale: float = 1.0):
    """Fill a GPU tensor with scalar[0] * scale."""
    if cuda is None:
        raise RuntimeError("GPU not available: cannot launch kernels")
    assert scalar.on_gpu, "Scalar tensor must be on GPU"
    n = int(np.prod(shape))
    kernel_src = """
    __global__ void fill_from_scalar_kernel(const float* scalar, float* out, int n, float scale) {
        int idx = threadIdx.x + blockIdx.x * blockDim.x;
        if (idx < n) out[idx] = scalar[0] * scale;
    }
    """
    func = get_kernel("fill_from_scalar_kernel", kernel_src)
    out_gpu = mempool.alloc(n * 4)
    bs = _optimal_block_size()
    stream = _get_stream()
    func(scalar.gpu_ptr, out_gpu, np.int32(n), np.float32(scale),
         block=(bs, 1, 1), grid=((n + bs - 1) // bs, 1, 1), stream=stream)
    Tensor = _get_tensor_cls()
    result = Tensor(out_gpu, gpu=True, inputs=[scalar])
...
def launch_relu_backward(inp, grad):
    """Compute grad * (inp > 0) on GPU."""
    if cuda is None:
        raise RuntimeError("GPU not available: cannot launch kernels")
    assert inp.on_gpu and grad.on_gpu, "Inputs must be on GPU"
    n = inp.size
    kernel_src = """
    __global__ void relu_backward_kernel(const float* inp, const float* grad, float* out, int n) {
        int idx = threadIdx.x + blockIdx.x * blockDim.x;
        if (idx < n) out[idx] = inp[idx] > 0.0f ? grad[idx] : 0.0f;
    }
    """
    func = get_kernel("relu_backward_kernel", kernel_src)
    out_gpu = mempool.alloc(n * 4)
    bs = _optimal_block_size()
    stream = _get_stream()
    func(inp.gpu_ptr, grad.gpu_ptr, out_gpu, np.int32(n),
         block=(bs, 1, 1), grid=((n + bs - 1) // bs, 1, 1), stream=stream)
    Tensor = _get_tensor_cls()
    result = Tensor(out_gpu, gpu=True, inputs=[inp, grad])
...
def launch_reduce_axis0(a, scale: float = 1.0):
    """Reduce a 2D GPU tensor over rows, returning shape (N,)."""
    if cuda is None:
        raise RuntimeError("GPU not available: cannot launch kernels")
    assert a.on_gpu and len(a.shape) == 2, "Input must be a 2D GPU tensor"
    M, N = a.shape
    kernel_src = """
    __global__ void reduce_axis0_kernel(const float* in, float* out, int M, int N, float scale) {
        int col = blockIdx.x;
        int tid = threadIdx.x;
        ...
        if (tid == 0) out[col] = smem[0] * scale;
    }
    """
    bs = 128
    func = get_kernel("reduce_axis0_kernel", kernel_src)
    out_gpu = mempool.alloc(N * 4)
    stream = _get_stream()
    func(a.gpu_ptr, out_gpu, np.int32(M), np.int32(N), np.float32(scale),
         block=(bs, 1, 1), grid=(N, 1, 1), shared=bs * 4, stream=stream)
    Tensor = _get_tensor_cls()
...
def launch_matmul_transpose(a, b, trans_a: bool = False, trans_b: bool = False):
    """Tiled matmul with optional logical transposes, for GPU backward paths."""
    if cuda is None:
        raise RuntimeError("GPU not available: cannot launch kernels")
    ...
    K2 = b_cols if trans_b else b_rows
    N = b_rows if trans_b else b_cols
    assert K == K2, f"Shape mismatch for transposed matmul: {a.shape} @ {b.shape}"
    ...
    __global__ void matmul_transpose_kernel(
        ...
        int M, int K, int N, int trans_a, int trans_b
    ) {{
        ...
        for (int t = 0; t < (K + TILE_SIZE - 1) / TILE_SIZE; t++) {{
            int ak = t * TILE_SIZE + threadIdx.x;
            int bk = t * TILE_SIZE + threadIdx.y;
            if (row < M && ak < K) {{
                As[threadIdx.y][threadIdx.x] = trans_a ? A[ak * A_cols + row] : A[row * A_cols + ak];
            }} else {{
                As[threadIdx.y][threadIdx.x] = 0.0f;
            }}
            if (bk < K && col < N) {{
                Bs[threadIdx.y][threadIdx.x] = trans_b ? B[col * B_cols + bk] : B[bk * B_cols + col];
            }} else {{
                Bs[threadIdx.y][threadIdx.x] = 0.0f;
            }}
            __syncthreads();
            for (int k = 0; k < TILE_SIZE; k++) acc += As[threadIdx.y][k] * Bs[k][threadIdx.x];
            __syncthreads();
        }}
        if (row < M && col < N) C[row * N + col] = acc;
    }}
    """
    func = get_kernel("matmul_transpose_kernel", kernel_src)
    out_gpu = mempool.alloc(M * N * 4)
    stream = _get_stream()
    func(a.gpu_ptr, b.gpu_ptr, out_gpu,
         np.int32(a_rows), np.int32(a_cols), np.int32(b_rows), np.int32(b_cols),
         np.int32(M), np.int32(K), np.int32(N), np.int32(1 if trans_a else 0), np.int32(1 if trans_b else 0),
         block=(TILE, TILE, 1), grid=((N + TILE - 1) // TILE, (M + TILE - 1) // TILE, 1), stream=stream)
    Tensor = _get_tensor_cls()
