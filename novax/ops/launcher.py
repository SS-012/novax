...
def launch_matmul_bias_mean(x, w, bias):
    """
    Fused matmul + bias + mean for no-grad MLP forward paths.
    x: (M, K), w: (K, N), bias: (N,) -> scalar mean.
    """
    if cuda is None:
        raise RuntimeError("GPU not available: cannot launch kernels")
    assert x.on_gpu and w.on_gpu and bias.on_gpu, "All tensors must be on GPU"
    ...
    kernel_src = f"""
    #define TILE_SIZE {TILE}
    __global__ void matmul_bias_mean_kernel(
    ...
    """
    func = get_kernel("matmul_bias_mean_kernel", kernel_src)
    out_gpu = mempool.alloc(4)
    stream = _get_stream()
    if stream is not None:
        cuda.memset_d32_async(out_gpu, 0, 1, stream)
